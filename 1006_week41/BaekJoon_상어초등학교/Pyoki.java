...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int num : order) {
					...
					for (int k = 0; k < 4; k++) {
						...
						if (isRange(nrow, ncol)) {
							...
							for (int likeNum : likePeople) {
								if (classRoom[nrow][ncol] == likeNum)adjCnt++;
							}
						}
					}
					//System.out.println(num+"------ad"+adjCnt+" em"+emptyCnt +" rc"+i+","+j);
			...
			//System.out.println("ERQERQRQE-----}}"+num+"------ad"+pick.adjCnt+" em"+pick.emptyCnt +" rc"+pick.row+","+pick.col);
		...
		for (int i = 1; i <= N; i++) {
			for (int j = 1; j <= N; j++) {
				...
				for (int k = 0; k < 4; k++) {
					...
					if (isRange(nrow, ncol)) {
						for (int likeNum : likePeople) {
							if (classRoom[nrow][ncol] == likeNum)happyCnt++;
						}
					}
				}
				
				if(happyCnt==1) answer+= 1;
				else if(happyCnt ==2) answer+=10;
				else if(happyCnt ==3) answer+=100;
				else if(happyCnt ==4) answer+=1000;
			}
		}
		return answer;
	}
